# Como grabar el GIF hero de NAC3 -- Guion en castellano
...
**Donde se va a usar:** como imagen principal del anuncio de lanzamiento publico (Show HN, X, LinkedIn).
...
4. La pagina te ofrece varios formatos. Elegi el que dice **"Installer"** (es el archivo `.exe`). Si te ofrece "Portable", tambien sirve pero te conviene el Installer porque crea un acceso directo.
...
10. Al final marca la opcion **"Launch ScreenToGif"** y apretá **"Finish"**.
...
2. Apretá las teclas **Ctrl + Shift + N** todas juntas. Eso abre una ventana de Chrome **en modo incognito** (esto es para que no haya extensiones interfiriendo).
3. En la barra de direcciones de la ventana incognito, escribi exactamente esta direccion:
   ```
...
### A3. Configurar el demo antes de grabar
...
2. **Elegi el codigo "en"** (ingles).

   **Aclaracion para el grabador:** lo vamos a grabar diciendo una frase en INGLES porque el video va a viajar internacionalmente (Hacker News, X). Aunque vos hables castellano nativo, esta grabacion necesita la frase en ingles. Si te incomoda, podes pedirle a alguien que hable mejor ingles que la diga, o usar la version en castellano del guion (otro archivo).
...
1. Achicá la ventana de Chrome a un tamano comodo: aproximadamente 1280 pixeles de ancho por 800 pixeles de alto. No es necesario que sea exacto, lo importante es que se vea bien la ventana + el modal + el chat panel sin que el browser este maximizado.
2. Posiciona la ventana en el centro de la pantalla. Las pestanas + barra de direcciones del browser quedan visibles arriba.
...
1. Si cerraste ScreenToGif, abrilo de nuevo (deberia estar en el menu inicio o en el escritorio).
...
3. **Cuando empieza a grabar** (vas a ver el contador subir en 0, 1, 2, ... segundos), hace EXACTAMENTE en este orden:
   ...
   d. **Segundo 3-8:** **deci en voz clara la siguiente frase en INGLES**:
      > **"Add a monitor, quantity one, price two hundred fifty."**
      
      Hablá calmadamente, claro, sin acelerar. Si te trabás, deja que se grabe igual y haces un segundo intento despues.
   e. **Segundo 8-10:** quedate quieto. El microfono se va a apagar solo o haces click otra vez en "mic" si sigue grabando. La transcripcion de lo que dijiste va a aparecer en el chat.
   f. **Segundo 10-12:** mira a la tabla. Tendría que aparecer una fila nueva al final que dice algo como "Monitor / 1 / 250".
...
### B5. Recortar el video si quedó muy largo

1. Si el video duró mas de 15 segundos, recortá los segundos iniciales o finales que sean redundantes.
...
2. Buscá `nac3-hero-voice-en.gif`.
...
1. Manda el archivo `nac3-hero-voice-en.gif` por email a `pablo.kuschnirof@gmail.com` con asunto **"GIF NAC3 hero -- listo"**.
