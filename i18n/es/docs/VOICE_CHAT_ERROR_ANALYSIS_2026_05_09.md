...
translation_lang: es
translation_date: 2026-05-11T13:33:39.784527+00:00
---
...
arquitectura; 1 es **un blocker de UX** (el modal bloquea el chat).
...
|---|---|---|---|
| 1 | "apreta describe_v20" | "No encuentro esa accion. Lo mas parecido: Correr el self-test, Ver capacidades, Ver mapa del sistema." | **C4** -- el manifiesto v20_panel no está en el árbol que recibe el backend. El bot busca en el árbol que ve y el top-3 son los slugs reales presentes. |
...
| 16 | "eliminatorias" | "Eliminando la factura." | **C2** -- el matcher hace sub-cadena: el prefijo "elimin" matchea "eliminar". Cualquier palabra que empiece con "elimina" dispara la acción. |
...
| 20 | "del sistema" | "Mostrando el mapa del sistema." | **C5** -- un fragmento del input previo se procesa como comando independiente. |
...
| 24 | "cambia el idioma de la página a chino" | "No encuentro un control de idioma en esta página..." | **C7** -- el LLM no reconoce este meta-comando como cambio de idioma de la sesión, sino que lo busca como un control en el árbol. |
...
`SpeechRecognition` está activo al mismo tiempo (modo manos
libres). El micrófono recoge la voz de los altavoces, el STT
...
1. **Half-duplex obligatorio** (la corrección estándar de la
   ...
   - Costo: el usuario no puede hablar ENCIMA del bot. Aceptable
     en el 99% de los casos; añade latencia percibida pero
     evita el loop.
...
**Recomendación:** implementar (1) de inmediato + (3) a corto
plazo. (2) es opcional para entornos donde el usuario quiere
...
- Mantener una lista corta de stems "comando" en el system
  prompt (~30 verbos) para acotar la heurística.
...
agregar una regla explícita: "verbos como `eliminar`, `borrar`,
...
1. El modal de confirmación debe estar **fuera del trap de
   ...
   **fuera del trap** del modal. Práctica recomendada: mover el chat a
...
modal NO encierre el chat en su árbol de foco.
...
`bootV20()` que hace polling con `setTimeout(bootV20, 50)` hasta que
`NAC.scope` exista. Si:
- el navegador es lento o el deploy del rc5 todavía no
  llegó (el rpaforce-crm vendoriza su propia copia de
...
**Evidencia:**
- "apreta describe_v20" -> el bot no encuentra
  `v20_panel.describe_v2`.
- "pulsa system map" -> el bot SÍ encuentra `navmap.fetch_map`
  (porque navmap se registra en el boot de example.js, mucho antes).

**Soluciones:**

1. **Migrar `nacDemoSnapshotTree` a `describe_v2()`** (cuando
   esté disponible). `describe_v2()` retorna tanto los
   v1_plugins (compat) como los v2_scope_entries -- garantiza que
   los manifests registrados via `NAC.register` Y los scopes
   declarados via `NAC.scope` lleguen al backend.
2. **Bloquear el envío del primer mensaje hasta que `bootV20()`
   complete.** El `chat-send` tiene un estado disabled hasta
...
**Recomendación:** combinar (1) + (3). (1) es el fix
...
1. **Buffer + debounce con timeout de silencio**:
   ...
   - Enviar al backend solo cuando hay 800-1500 ms de silencio
...
3. **Filtro contra C1**: si el transcript matchea (>70%) alguno de los
   últimos N utterances del bot, descartar.

**Recomendación:** (1) + (3). Es el estándar en aplicaciones de voz
...
"show / hide" como sinónimos, no solo "Toggle panel". Actualizar
el manifest:
...
Costo: 1 línea en el system prompt + 1 branch en el handler
del backend.
...
|---|---|---|---|---|
| 1 | Half-duplex TTS/STT (silenciar micrófono mientras habla el bot) | C1 | bajo | crítico |
| 2 | Confirmar acciones destructivas con `confirm_action()` | C1, C3 | medio | crítico |
...
| 6 | Mover `NAC.register('v20_panel')` al arranque temprano | C4 | trivial | medio |
...
Documentar también lo que funcionó bien para no romperlo:
...
críticas). Los fixes #2, #3, #5 pueden ir en un PR separado de
mayor envergadura. El resto puede quedar en el backlog.
...
| C4 | v20_panel no llega al chat | (a) Manifest extraído a `window.__V20_PANEL_MANIFEST__` y registrado vía `registerV20PanelManifest()` con polling de 30ms tan pronto como `NAC.register` exista (antes que `bootV20`). (b) `nacDemoSnapshotTree` ahora también incluye `v2_scope_entries`, `v2_intermediate_scopes`, `sitemap`, `tenant_prefix`, `nac_version_v2` cuando `NAC.describe_v2` existe. | `example-v20-full.php` (bloque de registro temprano); `js/example.js` `nacDemoSnapshotTree` extendido |
| C5 | Fragmentos STT como comandos | Buffer `_sttBuffer` + `setTimeout(_sttFlush, 1100)`. Cada resultado `final` de STT reinicia el timer; recién después de 1100ms de silencio el buffer se descarga al backend. Se limpia el buffer en el path manual (chatSend / mic-stop). | `js/example.js` `recognizer.onresult` + `_sttFlush` |
| C6 | "show" mapea mal | Resuelto al cerrar C4 (v20_panel ahora visible en el árbol). Adicional: `label_i18n.en` del v20_panel.toggle actualizado de "Toggle panel" a "Show or hide v2.0 panel" + 9 locales nuevos completos. | `example-v20-full.php` `__V20_PANEL_MANIFEST__` |
...
| C8 | Verbo en plugin equivocado (warning en consola "No action with verb=fetch_map found in plugin selftest") | Regla 12 explícita: "PLUGIN-VERB BINDING is fixed by the manifest. Do NOT guess, do NOT carry the verb to a nearby plugin, do NOT invent a plugin name." Con ejemplos de WRONG ↔ RIGHT. | `crm_desa/api/v1/yujin.php` `yjNacDemoSystemPrompt` regla 12 |

### Lo que NO modifiqué (intencional)

- **System prompt principal (sec 16 contract):** intacto. Solo
  agregué las reglas 11, 12, 13 como refinamientos; las
  absolutas A-F y las 1-10 no cambiaron.
- **Matcher local `interpret()`:** ya tokeniza por palabra
...
- **Confirm dialog (`NAC.confirm_dialog` en `nac.js`):** intacto;
  ...
  `data-nac-id`. Solo lo escucho ahora.
...
- **C1 nivel-3 (`confirm_action()` para destructivas):** sigue
  ...
  como destructiva (`data-nac-a11y-hint=destructive`) PASE primero
  por `confirm_dialog`. Lo dejo como follow-up: implica
  inspeccionar manifest.actions[].destructive y, si aplica,
  envolver el invoke con `confirm_action()` en la capa de dispatch.
- **STT debounce (C5):** los 1100ms son un valor empírico.
  Si se observa que "el bot tarda en responder a comandos cortos",
  bajar a 800ms y observar.
- **Filtro de feedback TTS (C1) -- nivel agresivo:** el threshold
  ...
  las capacidades"). Telemetría futura: contar cuántos drops
  registra `[stt] dropping bot-echo` -- si supera N por sesión,
  bajar el threshold a 80%.
